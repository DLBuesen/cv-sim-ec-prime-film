...
# Prepare the panel figure for the first time point. The plots are prepared individually and then combined before sending to the Electron window
    ...
    #-----Prepare the individual PoxPred plot
        ...
        PoxPred_layout = Layout(;title="Pox and Pred Dimensionless Concentration Profile <br> TimeIndex = $timeIndex of $numTimePts ",
    ...
    #-----Prepare the individual YoxZred plot

        Yox_trace = scatter(; x=dimensionless_x, y = Yox, mode="lines", name="Yox", line_color = "rgb(62, 13, 12)", line_width = "3")

        Zred_trace = scatter(; x=dimensionless_x, y = Zred, mode="lines", name="Zred", line_dash = "dash", line_color = "rgb(12, 62, 18)", line_width = "3")

        YoxZred_data = [Yox_trace, Zred_trace] ;

        YoxZred_layout = Layout(;title="Yox and Zred Dimensionless Concentration Profile <br> TimeIndex = $timeIndex of $numTimePts ",
            xaxis=attr(title="Dimensionless Distance"),
            yaxis=attr(title="Dimensionless Concentration")
            )

        YoxZred_Plot = plot(YoxZred_data, YoxZred_layout)
    ...
    #-----Prepare the first frame of the panel plot
		...
		dlessConcProfilePanel_Plot =  [dlessPotential_Plot dlessFarCurr_Plot ; PoxPred_Plot YoxZred_Plot] ;
...
# Update the plot for all time points in a loop
    ...
    for i=2:numTimePts
          ...
          Yox_loop = sol[1,YoxSpaceIndexStart:YoxSpaceIndexEnd,timeIndex_loop] ;
          Zred_loop = 1.0 .- Yox_loop ;
          ...
          # Update the YoxZred plot title and traces

                relayout!(YoxZred_Plot, title="Yox and Zred Dimensionless Concentration Profile <br> TimeIndex = $timeIndex_loop of $numTimePts ")

                # Update the concentration profile traces

                traceNumberToUpdate = 1 ;
                restyle!(YoxZred_Plot, [traceNumberToUpdate], y = [Yox_loop])

                traceNumberToUpdate = 2 ;
                restyle!(YoxZred_Plot, [traceNumberToUpdate], y = [Zred_loop])
